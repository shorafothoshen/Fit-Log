...
import PlanItemCard from "@/app/components/planComponents/planCard";
import Link from "next/link";

const Page = () => {
  const { TodayPlan, sortBy,setIsManuActive } = useContext(PlansContext);
  ...
  return (
    <div className="flex flex-col gap-4">
      {sortedPlan.map((workout) => (
        <PlanItemCard key={workout.id} workout={workout} />
      ))}
    </div>
  );
};
